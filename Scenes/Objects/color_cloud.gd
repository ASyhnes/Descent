...
# effet elastique (calcul en pixel)
...
func _process(delta: float) -> void:
	...
	# permet de limiter l'etirement pour que le nuage ne part pas trop loin.
	total_stretch = total_stretch.limit_length(max_stretch)
	
	# On l'applique à la position cible
	target_pos += total_stretch
	
	# On déplace la zone de détection physique (PerceptionArea) avec le nuage
	if perception_area:
